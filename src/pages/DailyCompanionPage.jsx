        ...
        <div className="bg-[#f8f7f6] dark:bg-[#1b130d] font-display text-slate-900 dark:text-slate-100 transition-colors duration-300 min-h-screen">
            <div className="relative flex h-auto min-h-screen w-full flex-col overflow-x-hidden">
                <div className="layout-container flex h-full grow flex-col">
                    {/* Header/Navigation */}
                    <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-[#ee7c2b]/10 px-10 py-4 bg-[#f8f7f6]/80 dark:bg-[#1b130d]/80 backdrop-blur-md sticky top-0 z-50">
                        <div className="flex items-center gap-8">
                            <button
                                onClick={() => navigate(l('/companion'))}
                                className="flex items-center gap-3 text-[#ee7c2b] hover:opacity-80 transition-opacity"
                            >
                                <div className="size-8 bg-[#ee7c2b] rounded-lg flex items-center justify-center text-white">
                                    <span className="material-symbols-outlined">smart_toy</span>
                                </div>
                                <h2 className="text-slate-900 dark:text-slate-100 text-xl font-bold leading-tight tracking-tight">AI Companion</h2>
                            </button>
                            <nav className="hidden md:flex items-center gap-8">
                                ...
                                <button onClick={() => navigate(l('/companion/daily'))} className="text-[#ee7c2b] text-sm font-semibold leading-normal border-b-2 border-[#ee7c2b] pb-1">日常陪伴</button>
                        ...
                        <div className="flex flex-1 justify-end gap-6 items-center">
                            ...
                            <button onClick={() => navigate(l('/login'))} className="flex min-w-[84px] cursor-pointer items-center justify-center rounded-xl h-10 px-6 bg-[#ee7c2b] text-white text-sm font-bold hover:opacity-90 transition-all shadow-lg shadow-[#ee7c2b]/20">
                                <span className="truncate">登录</span>
                            </button>
                            <div className="bg-center bg-no-repeat aspect-square bg-cover rounded-full size-10 ring-2 ring-[#ee7c2b]/20" style={{ backgroundImage: 'url("https://lh3.googleusercontent.com/aida-public/AB6AXuDU0w_siZ42eKX4bX3jo_ZbsmPOmW8Yzj9mBUE7lV27pRD4Q_Hsd_VNSSvOS_xr5921Q6rI0OXZWzL10S9Oj6DgrmIQMzBVY7SFLdSS6GP5CXB2JlWWx1RxdNSXAau0OntW6F-3m5unAJCtw48sI9lJoEC6x_Jac94FTlY_CgcbL4LHvMX3PS79laJIEjAKDXg4qNqq92itzl4gsfydmg8aqWr5sDnDnULPaXXilSsi7q6e5uoyBklsiq6DV8bAeS0BjqLRN2a945s")' }}></div>
                        </div>
                    </header>

                    <main className="flex flex-col items-center w-full max-w-7xl mx-auto px-6 py-12">
                        {/* Hero Section */}
                        <div className="w-full flex flex-col md:flex-row justify-between items-end gap-6 mb-12">
                            <div className="flex flex-col gap-4 max-w-2xl text-left">
                                <h1 className="text-slate-900 dark:text-slate-100 text-5xl font-black leading-tight tracking-tight">选择您的AI伴侣</h1>
                                <p className="text-slate-600 dark:text-slate-400 text-lg font-normal">寻找懂你的数字伙伴，开启每日温馨陪伴。无论是倾听心事，还是分享快乐，他们永远在这里。</p>
                            </div>
                        </div>
                        ...
                        <div className="w-full bg-white dark:bg-slate-900/50 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 mb-10">
                            <div className="flex flex-col gap-6 font-display">
                                <label className="flex flex-col w-full h-14">
                                    <div className="flex w-full flex-1 items-stretch rounded-xl h-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 focus-within:border-[#ee7c2b]/50">
                                        <div className="text-slate-400 flex items-center justify-center pl-4 rounded-l-xl">
                                            <span className="material-symbols-outlined text-[24px]">search</span>
                                        </div>
                                        <input className="form-input flex w-full min-w-0 flex-1 border-none bg-transparent focus:ring-0 h-full placeholder:text-slate-400 px-4 pl-2 text-lg font-normal" placeholder="搜索AI伴侣名称或性格关键词 (如：温柔、幽默)" />
                                        <button className="bg-[#ee7c2b] text-white px-8 rounded-r-xl font-bold hover:bg-[#ee7c2b]/90 transition-colors">搜索</button>
                                    </div>
                                </label>
                                <div className="flex gap-3 flex-wrap items-center">
                                    <span className="text-sm font-bold text-slate-500 mr-2">筛选条件:</span>
                                    <button className="flex h-10 shrink-0 items-center justify-center gap-x-2 rounded-xl bg-[#ee7c2b] text-white px-5">
                                        <span className="text-sm font-semibold">全部</span>
                                        <span className="material-symbols-outlined text-[18px]">expand_more</span>
                                    </button>
                                    <button className="flex h-10 shrink-0 items-center justify-center gap-x-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-5 hover:bg-slate-200 transition-colors">
                                        <span className="text-sm font-semibold">女性</span>
                                        <span className="material-symbols-outlined text-[18px]">female</span>
                                    </button>
                                    <button className="flex h-10 shrink-0 items-center justify-center gap-x-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-5 hover:bg-slate-200 transition-colors">
                                        <span className="text-sm font-semibold">男性</span>
                                    ...
                                    <button className="flex h-10 shrink-0 items-center justify-center gap-x-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-5 hover:bg-slate-200 transition-colors">
                                        <span className="text-sm font-semibold">甜美系</span>
                                    </button>
                                    <button className="flex h-10 shrink-0 items-center justify-center gap-x-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-5 hover:bg-slate-200 transition-colors">
                                        <span className="text-sm font-semibold">高冷系</span>
                                    </button>
                                    <button className="flex h-10 shrink-0 items-center justify-center gap-x-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 px-5 hover:bg-slate-200 transition-colors">
                                        <span className="text-sm font-semibold">知性</span>
                                    </button>
                                </div>
                            </div>
                        </div>
